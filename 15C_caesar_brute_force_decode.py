...
def caesar_decode(string, shift):
    # copied here from 15C_caesar_cypher.py
    result = ''
    for char in string:
        if char == ' ':
            result += ' '
...
for encoded, decoded in inputs:
    print(encoded)
    print(caesar_brute_decode(encoded))
    print('correct' if decoded == caesar_brute_decode(encoded) \
          else 'incorrect')
print() # to separate results from next solution
...
LETTER_GOODNESS = [
    .0817, .0149, .0278, .0425, .1270, # A - E
    .0223, .0202, .0609, .0697, .0015, # F - J
    .0077, .0402, .0241, .0675, .0751, # K - O
    .0193, .0009, .0599, .0633, .0906, # P - T
    .0276, .0098, .0236, .0015, .0197, # U - Y
    .0007                              # Z
]

def caesar_decode(string, shift):
    # copied from 15C_caesar_cypher.py
...
def string_goodness(string, n):
    ...
    for char in string:
        if char == ' ':
            ...
            char = caesar_decode(char, n)
            total_goodness += LETTER_GOODNESS[ord(char) - ord('A')]

    return total_goodness

def probable_shift_value(encoded):
    goodness_for_each_shift = []

    for n in range(26):
        goodness_for_each_shift.append(string_goodness(encoded, n))
    
    return goodness_for_each_shift.index(max(goodness_for_each_shift))

for encoded, decoded in inputs:
    print(encoded)
    print(caesar_decode(encoded, probable_shift_value(encoded)))
    print('correct' if decoded == \
          caesar_decode(encoded, probable_shift_value(encoded)) \
          else 'incorrect')
